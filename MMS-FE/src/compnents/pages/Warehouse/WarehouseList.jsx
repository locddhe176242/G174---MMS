...
export default function WarehouseList() {
	...
	const [confirmState, setConfirmState] = useState({
		open: false,
		action: null, // 'deactivate' | 'restore'
		warehouseId: null,
		message: "",
	});
	...
	const fetchWarehouses = async (page = 0, keyword = "") => {
		try {
        setLoading(true);
			setError(null);
			...
			let response;
			if (keyword.trim()) {
				response = await warehouseService.searchWarehousesWithPagination(keyword, page, pageSize);
			} else {
				response = await warehouseService.getWarehousesWithPagination(page, pageSize);
			}
	...
	const handleSearch = (e) => {
		e.preventDefault();
		fetchWarehouses(0, searchKeyword);
	};

	const handlePageChange = (newPage) => {
		fetchWarehouses(newPage, searchKeyword);
    };
	...
	const performConfirmedAction = async () => {
		...
		try {
			...
			fetchWarehouses(currentPage, searchKeyword);
		} catch (err) {
			...
			<div className="bg-white shadow-sm">
				<div className="container mx-auto px-4 py-6">
					<div className="flex items-center justify-between">
						...
						<Link to="/warehouse/new" className="bg-black text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-800 transition-colors">
						...
						<div className="fixed inset-0 z-50 flex items-center justify-center">
							...
							<div className="relative bg-white rounded-lg shadow-lg w-full max-w-md mx-4">
								...
								<div className="px-6 py-4 border-t border-gray-200 flex items-center justify-end gap-3">
									<button onClick={closeConfirm} className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50">Hủy</button>
									<button onClick={performConfirmedAction} className="px-4 py-2 rounded-lg bg-black text-white hover:bg-gray-800">Xác nhận</button>
								</div>
							</div>
						</div>
					)}
								...
								<tbody className="bg-white divide-y divide-gray-200">
									{warehouses.map((w) => (
										<tr key={w.warehouseId} className="hover:bg-gray-50">
											...
											<td className="px-6 py-4 whitespace-nowrap text-sm">
												<span
													className={`px-2 py-1 rounded-full text-xs ${w.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{w.status}</span>
											</td>
											<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(w.createdAt)}</td>
											<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{w.createdBy?.email || ""}</td>
											<td className="px-6 py-4 whitespace-nowrap text-sm">
						...
						<div className="px-6 py-4 border-t border-gray-200">
							<div className="flex items-center justify-between">
								<div className="text-sm text-gray-700">
									Hiển thị {currentPage * pageSize + 1}-{Math.min((currentPage + 1) * pageSize, totalElements)} trong tổng số {totalElements} kho
								</div>
								...
								<div className="flex items-center gap-2">
									...
									{Array.from({ length: totalPages }, (_, i) => (
										<button key={i} onClick={() => handlePageChange(i)} className={getPaginationButtonClass(i === currentPage)}>
											{i + 1}
										</button>
									))}
