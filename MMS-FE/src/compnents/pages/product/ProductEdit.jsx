...
import { X, Upload, Image as ImageIcon, Save, Sparkles } from 'lucide-react';
import { getCategories } from '../../../api/categoryService'; // Nếu dùng API
import { updateProduct } from '../../../api/productService'; // Gọi API update nếu cần
...
const ProductEdit = ({ product, onClose, onSave }) => {
    ...
    const fetchCategories = async () => {
            try {
                ...
                if (product?.categoryId) {
                    // Kiểm tra xem categoryId có tồn tại trong data không
                    const catExists = data.find(c => c.id === product.categoryId);
                    if (catExists) {
                        setFormData(prev => ({ ...prev, categoryId: product.categoryId }));
                    }
                } else if (product?.category_name) {
                    const category = data.find(c => c.name === product.category_name);
                    if (category) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            onSave(response); // cập nhật state parent
            onClose();
        } catch (error) {
            ...
            <div className="w-full max-w-5xl max-h-[90vh] overflow-hidden bg-white rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600 mt-0.5">Cập nhật thông tin sản phẩm</p>
                            </div>
                        </div>
                        <button onClick={onClose} className="p-2 hover:bg-white/80 rounded-xl transition-all duration-200 group">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <div className="flex-1 overflow-y-auto px-8 py-6" style={{ maxHeight: 'calc(90vh - 180px)' }}>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-6">
                                ...
                                {/* Thông tin khác */}
                                <div className="bg-gradient-to-br from-gray-50 to-green-50/30 rounded-xl p-6 border border-gray-200/50">
                                    <h3 className="text-base font-semibold text-gray-800 mb-5">Thông tin khác</h3>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Đơn vị đo</label>
                                            <input type="text" name="uom" value={formData.uom} onChange={handleChange} className="w-full px-4 py-2.5 border rounded-xl" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Size</label>
                                            <input type="text" name="size" value={formData.size} onChange={handleChange} className="w-full px-4 py-2.5 border rounded-xl" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">SKU</label>
                                            <input type="text" name="sku" value={formData.sku} onChange={handleChange} className="w-full px-4 py-2.5 border rounded-xl" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Quantity</label>
                                            <input type="number" name="quantity" value={formData.quantity} onChange={handleChange} className="w-full px-4 py-2.5 border rounded-xl" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Trạng thái</label>
                                            <select name="status" value={formData.status} onChange={handleChange} className="w-full px-4 py-2.5 border rounded-xl cursor-pointer">
                    ...
                    <div className="flex items-center justify-end gap-3 px-8 py-5 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-blue-50/30">
                        <button type="button" onClick={onClose} className="px-6 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl">Hủy</button>
                        <button type="submit" disabled={loading} className="px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl flex items-center gap-2">
