...
export default function APInvoiceList() {
  ...
  const fetchInvoices = async (page = 0, keyword = "", sortFieldValue = "createdAt", sortDirectionValue = "desc") => {
    try {
      ...
      let response;
      if (keyword.trim()) {
        response = await apInvoiceService.searchInvoicesWithPagination(keyword, page, pageSize, sort);
      } else {
        response = await apInvoiceService.getInvoicesWithPagination(page, pageSize, sort);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      await apInvoiceService.deleteInvoice(invoiceToDelete.ap_invoice_id || invoiceToDelete.id);
      ...
      <table className="w-full">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              <button onClick={() => handleSort("ap_invoice_id")} className="flex items-center gap-1 hover:text-gray-700">
                ID
                {getSortIcon("ap_invoice_id")}
              </button>
            </th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {invoices.map((invoice) => (
            <tr key={invoice.ap_invoice_id || invoice.id} className="hover:bg-gray-50">
              <td className="px-6 py-4">
                <input type="checkbox" className="rounded border-gray-300" />
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                {(invoice.ap_invoice_id || invoice.id)?.toString().padStart(4, "0")}
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => navigate(`/purchase/ap-invoices/${invoice.ap_invoice_id || invoice.id}`)}
                  ...
                  <button
                    onClick={() => navigate(`/purchase/ap-invoices/${invoice.ap_invoice_id || invoice.id}/edit`)}
