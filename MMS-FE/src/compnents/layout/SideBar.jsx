  ...
  useEffect(() => {
    ...
    const MenuItem = ({item}) => {
            ...
            return (
                <div>
                    ...
                    >
                        ...
                        {item.badge && (
                            <span
                                className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{item.badge}</span>
                        )}
                        <svg
                        ...
                        <div className="mt-1 ml-8 space-y-1">
                            {item.children.map((child) => (
                                ...
                                >
                                    ...
                                    {child.badge && (
                                        <span
                                            className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{child.badge}</span>
                                    )}
                                </Link>
                            ))}
                        </div>
        ...
        if (hasChildren && isCollapsed) {
            return (
                ...
                >
                    ...
                    <div
                        className="absolute left-full ml-2 px-3 py-2 bg-slate-800 text-white text-sm rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50">
                        {item.label}
                        {item.children && (
                            <div
                                className="mt-1 text-xs text-slate-300">{item.children.map((c) => c.label).join(", ")}</div>
                        )}
                    </div>
                </button>
            );
        ...
        return (
            ...
            >
                <span
                    className={`w-5 h-5 flex-shrink-0 ${isActive ? "text-white" : "text-brand-blue"}`}>{item.icon}</span>
        ...
        >
            <div className="p-4">
                {menuConfig.mainMenu && menuConfig.mainMenu.length > 0 && (
                    <div className="mb-4">
                        {!isCollapsed && (
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3 px-3">Menu
                                chính</div>
                        )}
                ...
                {menuConfig.managementMenu && menuConfig.managementMenu.length > 0 && (
                    <div className="pt-4 border-t border-slate-200">
                        {!isCollapsed && (
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3 px-3">Quản
                                lý</div>
                        )}
                        <nav className="space-y-1">
